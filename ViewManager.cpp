...
#include <GL/glew.h>        // GLEW (must come before other GL headers)
...
#include <glm/gtc/matrix_transform.hpp> // for glm::perspective / glm::ortho
...
{
	// First mouse movement code to record 
	// and use for later calculation for the offset
	if (gFirstMouse)
	{
 ...
 *
 *  NOTE: removed deprecated fixed-function projection calls
 *  (glMatrixMode / gluPerspective / glOrtho). Projection
 *  choice is now signaled via bOrthographicProjection and
 *  handled in PrepareSceneView using glm.
 ***********************************************************/
void ViewManager::ProcessKeyboardEvents()
{
	...
	// Debounced projection toggle for P (perspective) and O (orthographic)
	...
	{
		// switch to perspective projection (handled in PrepareSceneView)
		bOrthographicProjection = false;
	}
	pWasPressed = (pState == GLFW_PRESS);

	int oState = glfwGetKey(m_pWindow, GLFW_KEY_O);
	if (oState == GLFW_PRESS && !oWasPressed)
	{
		// switch to orthographic projection (handled in PrepareSceneView)
		bOrthographicProjection = true;
	}
	oWasPressed = (oState == GLFW_PRESS);
}
...
void ViewManager::PrepareSceneView()
{
	...
	{
		// Orthographic projection - match aspect ratio, keep reasonable volume
		const float orthoScale = 2.0f; // tweak as needed for scene scale
